...
int lastIdx = 0;     // index for the next jobs to be added in jobsList
...
void childRemove(int jobsList[], int *lastIdx, int pid)
{
    ...
    *lastIdx = *lastIdx - 1;
    ...
    for (int j = i; j < *lastIdx; j++) // update the rest
    {
...
void fgCmdP(int idx)
{
    fgChildPid = jobsList[idx]; // update process running in foreground
...
void fgCmdN()
{
    fgChildPid = jobsList[0]; // update process running in foreground
...
int main(void)
{
    ...
    while (1)
    {
        if (signal(SIGTSTP, SIG_IGN) == SIG_ERR) // ignore signal ctrl z
        {
            printf("ERROR! Could not bind the signal hander.\n");
            exit(1);
        }

        signal(SIGCHLD, cSigHandler); // catch signal upon child process termination

        bg = 0;        // 0: run in fg; 1: run in bg
        int rdFlg = -1; // -1: no redirection; >0: index of > sign
        int newFd;     // value of new file descriptor for redirection

        int fd[2];                            // fd used for piping
        int cnt = getcmd("\n>> ", args, &bg); // cnt: number of tokens

        if (bg == 0) // only process in foreground can be terminated by ctrl c
        {
        ...
        else // ignore SIGINT if process is running in background
        {
        ...
        else if (strcmp(cmd, "fg") == 0)
        {
            if (cnt > 1) // fg takes a param
            {
                ...
                else // remove specified process
                {
                    fgCmdP(idx);
                }
            }
            else if (lastIdx > 0) // fg takes no param & jobsList is not empty, then remove the first
            {
                fgCmdN();
            }
            free(linecp); // free memory allocated by malloc from line in getcmd()
        ...
        for (int i = 0; i < cnt; i++)
        {
            if (*(args[i]) == '>')
            {
                ...
                if (i == 0)
                {
                    printf("Error: Invalid input for redirection.\n");
                }
                else
                {
                    newFd = redirect(args, rdFlg);
                    break;
                }
            }
        }

        if (rdFlg == 0) // invalid input, go to the next iteration
        {
            free(linecp); // free memory allocated by malloc from line in getcmd()
            continue;
        }
        ...
        int ppFlg = -1; // -1: no piping; >0: index of vertical bar
        if (rdFlg == -1)
        {
            ppFlg = checkPipe(cnt, args);
        }
        if (ppFlg > 0) // pipe found
        {
            if (pipe(fd) < 0)
            {
            ...
            // first child, execute command before pipe
            int pid = fork();
            if (pid < 0)
            {
            ...
            // second child, execute command after pipe
            int pid2 = fork();
            if (pid2 < 0)
            {
                printf("Forking failed.\n");
        ...
        if (pid > 0 && bg == 1) // add jobs running in background to jobsList
        {
        ...
        else if (pid == 0) // child process
        {
            ...
            // for commands other than piping
        ...
        if (rdFlg > 0) // restore stdout after rediraction
        {
            dup2(newFd, STDOUT_FILENO);
        }

        if (bg == 0) // parent will wait if process is running in foreground
        {
            wait(NULL);
        }
