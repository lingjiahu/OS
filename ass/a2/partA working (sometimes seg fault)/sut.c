...
void sut_exit()
{
    ...
    if (!(queue_peek_front(&readyQ)) && !(queue_peek_front(&waitQ)) && exitFlg == 1 && shutdownFlg == 1) // both queues are empty, all tasks are done
    {
        ...
        if (numCExec == 2 && (pthread_equal(pthread_self(), cThread) != 0))
        {
        ...
        else if (numCExec == 2 && (pthread_equal(pthread_self(), cThread2) != 0))
        {
            if (pthread_cancel(cThread) != 0)
            {
                printf("cancel t1 errno: %d\n", errno);
