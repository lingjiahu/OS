...
int numCExec = 1; // global variable, number of C-EXEC available

pthread_t cThread;
pthread_t cThread2; // only used for 2 C-EXEC mode
pthread_t iThread;

threaddesc *tdescptrList[40]; // list of all tasks created, maximum 30 tasks
int numTasks;                 // number of tasks created
int numThreads = 0;           //number of tasks exited

pthread_mutex_t cMutex = PTHREAD_MUTEX_INITIALIZER;          // prevent race condition on readQ
pthread_mutex_t iMutex = PTHREAD_MUTEX_INITIALIZER;          // prevent race condition on waitQ
pthread_mutex_t numThreadsMutex = PTHREAD_MUTEX_INITIALIZER; // protect the record of total tasks created and remaining

struct queue readyQ; // store context of cpu tasks
struct queue waitQ;  // store context of io tasks

ucontext_t cContext;  // context of C-EXEC thread
ucontext_t cContext2; // context of second C-EXEC thread for 2 C-EXEC mode
ucontext_t iContext;  // context of I-EXEC thread

bool exitFlg = false; // exitFlg = true: no more task to be run, ready to shutdown

// function for first C-EXEC
void *c_exec()
{
    struct queue_entry *head;   // the next task to be executed
    ...
    while (true)
    {
        ...
        if (head) // there exists a task in ready queue
        {
            ...
            free(head); // deallocate memory allocated for queue entry
        }
        else
        {
...
// function for second C-EXEC, same logic as c_exec()
void *c_exec2()
{
...
// function for I-EXEC, same logic as c_exec()
void *i_exec()
{
    if (getcontext(&iContext) != 0)
    {
        printf("getcontext in i_exec failed\n");
    }
...
void sut_yield()
{
    ...
    if (pthread_equal(pthread_self(), cThread)) // swap back to the caller thread
    {
...
void sut_exit()
{
    ...
    if (!(queue_peek_front(&readyQ)) && !(queue_peek_front(&waitQ)) && noMoreTask) // both queues are empty, all tasks are done
    {
        exitFlg = true;
    }
...
void sut_shutdown()
{
    ...
    // free all memory allocated before shutdown
